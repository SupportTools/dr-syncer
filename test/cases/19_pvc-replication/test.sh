...
function verify_replication() {
  ...
  kubectl --context="${CTX_REMOTE}" annotate namespacemapping pvc-replication dr-syncer.io/trigger-sync=true
...
function cleanup() {
  ...
  kubectl --context="${CTX_REMOTE}" delete namespacemapping pvc-replication 2>/dev/null || true
