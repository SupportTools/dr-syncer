...
wait_for_replication() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        PHASE=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.phase}' 2>/dev/null)
        REPLICATION_STATUS=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.conditions[?(@.type=="Synced")].status}' 2>/dev/null)
...
main() {
    ...
    if verify_resource "dr-sync-test-case10" "configmap" "test-configmap"; then
        if verify_configmap "dr-sync-test-case10" "test-configmap"; then
    ...
    if verify_resource "dr-sync-test-case10" "persistentvolumeclaim" "test-pvc-standard"; then
        if verify_pvc "dr-sync-test-case10" "test-pvc-standard"; then
    ...
    if verify_resource "dr-sync-test-case10" "persistentvolumeclaim" "test-pvc-block"; then
        if verify_pvc "dr-sync-test-case10" "test-pvc-block"; then
    ...
    if verify_resource "dr-sync-test-case10" "persistentvolumeclaim" "test-pvc-filesystem"; then
        if verify_pvc "dr-sync-test-case10" "test-pvc-filesystem"; then
    ...
    if verify_resource "dr-sync-test-case10" "deployment" "test-deployment"; then
        if verify_deployment "dr-sync-test-case10" "test-deployment"; then
    ...
    if verify_resource "dr-sync-test-case10" "service" "test-service"; then
        if verify_service "dr-sync-test-case10" "test-service"; then
    ...
    # Get status fields
    local phase=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.phase}')
    local synced_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.syncStats.successfulSyncs}')
    local failed_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.syncStats.failedSyncs}')
    local last_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.lastSyncTime}')
    local next_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.nextSyncTime}')
    local sync_duration=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.syncStats.lastSyncDuration}')
    ...
    local resource_status_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync -o jsonpath='{.status.resourceStatus[*].status}' | tr ' ' '\n' | grep -c "Synced" || echo "0")
    ...
    local columns=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-basic-sync)
