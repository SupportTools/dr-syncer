...
wait_for_replication() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        PHASE=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.phase}' 2>/dev/null)
        REPLICATION_STATUS=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.conditions[?(@.type=="Synced")].status}' 2>/dev/null)
...
deploy_resources() {
    ...
    kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} annotate namespacemapping -n dr-syncer test01-standard-resources-wildcard dr-syncer.io/sync-now=true --overwrite
}

# Main test function
main() {
    echo "Testing DR-Sync functionality for case 01 (Wildcard Namespace Selection)..."
    ...
    if verify_resource "" "namespace" "dr-sync-test-case01"; then
    ...
    # Verify ConfigMap
    if verify_resource "dr-sync-test-case01" "configmap" "test-configmap"; then
        if verify_configmap "dr-sync-test-case01" "test-configmap"; then
    ...
    # Verify Secret
    if verify_resource "dr-sync-test-case01" "secret" "test-secret"; then
        if verify_secret "dr-sync-test-case01" "test-secret"; then
    ...
    # Verify Deployment
    if verify_resource "dr-sync-test-case01" "deployment" "test-deployment"; then
        if verify_deployment "dr-sync-test-case01" "test-deployment"; then
    ...
    # Verify Service
    if verify_resource "dr-sync-test-case01" "service" "test-service"; then
        if verify_service "dr-sync-test-case01" "test-service"; then
    ...
    # Verify Ingress
    if verify_resource "dr-sync-test-case01" "ingress" "test-ingress"; then
        if verify_ingress "dr-sync-test-case01" "test-ingress"; then
    ...
    # Get status fields
    local phase=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.phase}')
    local synced_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.syncStats.successfulSyncs}')
    local failed_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.syncStats.failedSyncs}')
    local last_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.lastSyncTime}')
    local next_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.nextSyncTime}')
    local sync_duration=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard -o jsonpath='{.status.syncStats.lastSyncDuration}')
    ...
    local columns=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test01-standard-resources-wildcard)
