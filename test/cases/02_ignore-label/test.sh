...
verify_resource_not_exists() {
    ...
    if kubectl --kubeconfig ${DR_KUBECONFIG} -n ${namespace} get ${resource_type} ${resource_name} &> /dev/null; then
        echo "Resource ${resource_type}/${resource_name} exists but should be ignored"
        return 1
    fi
    return 0
}
...
wait_for_replication() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        PHASE=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.phase}' 2>/dev/null)
        REPLICATION_STATUS=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.conditions[?(@.type=="Synced")].status}' 2>/dev/null)
...
deploy_resources() {
    ...
    kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} annotate namespacemapping -n dr-syncer test02-ignore-label-test dr-syncer.io/sync-now=true --overwrite
}

# Main test function
main() {
    echo "Testing DR-Sync functionality for case 02 (Ignore Label)..."
    ...
    if verify_resource "" "namespace" "dr-sync-test-case02"; then
    ...
    # Verify ConfigMap
    if verify_resource "dr-sync-test-case02" "configmap" "test-configmap"; then
        if verify_configmap "dr-sync-test-case02" "test-configmap"; then
    ...
    # Verify Secret
    if verify_resource "dr-sync-test-case02" "secret" "test-secret"; then
        if verify_secret "dr-sync-test-case02" "test-secret"; then
    ...
    if verify_resource "dr-sync-test-case02" "deployment" "test-deployment-1"; then
        if verify_deployment "dr-sync-test-case02" "test-deployment-1"; then
            print_result "Non-ignored deployment fully verified" "pass"
        else
            print_result "Non-ignored deployment verification failed" "fail"
        fi
    else
        print_result "Non-ignored deployment not found" "fail"
    fi
    
    # Verify Deployment 2 (should NOT be synced due to ignore label)
    if verify_resource_not_exists "dr-sync-test-case02" "deployment" "test-deployment-2"; then
        print_result "Ignored deployment correctly excluded" "pass"
    else
        print_result "Ignored deployment incorrectly synced" "fail"
    fi
    
    # Verify Service
    if verify_resource "dr-sync-test-case02" "service" "test-service"; then
        if verify_service "dr-sync-test-case02" "test-service"; then
    ...
    # Verify Ingress
    if verify_resource "dr-sync-test-case02" "ingress" "test-ingress"; then
        if verify_ingress "dr-sync-test-case02" "test-ingress"; then
    ...
    # Get status fields
    local phase=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.phase}')
    local synced_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.syncStats.successfulSyncs}')
    local failed_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.syncStats.failedSyncs}')
    local last_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.lastSyncTime}')
    local next_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.nextSyncTime}')
    local sync_duration=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test -o jsonpath='{.status.syncStats.lastSyncDuration}')
    ...
    local columns=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test02-ignore-label-test)
