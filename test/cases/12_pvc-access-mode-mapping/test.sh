...
verify_pvc() {
    local namespace=$1
    local name=$2
    local source_access_mode=$3
    local expected_dr_access_mode=$4
...
wait_for_replication() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        PHASE=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.phase}' 2>/dev/null)
        REPLICATION_STATUS=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.conditions[?(@.type=="Synced")].status}' 2>/dev/null)
...
main() {
    ...
    if verify_resource "dr-sync-test-case12" "configmap" "test-configmap"; then
        if verify_configmap "dr-sync-test-case12" "test-configmap"; then
    ...
    if verify_resource "dr-sync-test-case12" "persistentvolumeclaim" "test-pvc-rwo"; then
        if verify_pvc "dr-sync-test-case12" "test-pvc-rwo" "ReadWriteOnce" "ReadWriteOnce"; then
    ...
    if verify_resource "dr-sync-test-case12" "persistentvolumeclaim" "test-pvc-rwm"; then
        if verify_pvc "dr-sync-test-case12" "test-pvc-rwm" "ReadWriteMany" "ReadWriteMany"; then
    ...
    # Verify ReadOnlyMany PVC (mapped to ReadWriteMany)
    if verify_resource "dr-sync-test-case12" "persistentvolumeclaim" "test-pvc-rom"; then
        if verify_pvc "dr-sync-test-case12" "test-pvc-rom" "ReadOnlyMany" "ReadWriteMany"; then
            print_result "ReadOnlyMany PVC synced and verified" "pass"
        else
    ...
    # Verify ReadWriteOncePod PVC (mapped to ReadWriteOnce)
    if verify_resource "dr-sync-test-case12" "persistentvolumeclaim" "test-pvc-rwop"; then
        if verify_pvc "dr-sync-test-case12" "test-pvc-rwop" "ReadWriteOncePod" "ReadWriteOnce"; then
            print_result "ReadWriteOncePod PVC synced and verified" "pass"
        else
    ...
    if verify_resource "dr-sync-test-case12" "deployment" "test-deployment"; then
        if verify_deployment "dr-sync-test-case12" "test-deployment"; then
    ...
    if verify_resource "dr-sync-test-case12" "service" "test-service"; then
        if verify_service "dr-sync-test-case12" "test-service"; then
    ...
    # Get status fields
    local phase=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.phase}')
    local synced_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.syncStats.successfulSyncs}')
    local failed_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.syncStats.failedSyncs}')
    local last_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.lastSyncTime}')
    local next_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.nextSyncTime}')
    local sync_duration=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.syncStats.lastSyncDuration}')
    ...
    local resource_status_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping -o jsonpath='{.status.resourceStatus[*].status}' | tr ' ' '\n' | grep -c "Synced" || echo "0")
    ...
    local columns=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get replication pvc-access-mode-mapping)
