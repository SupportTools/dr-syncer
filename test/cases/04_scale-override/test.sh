...
# Function to verify Deployment with scale override check
verify_deployment() {
    local namespace=$1
    local name=$2
    ...
    # Get DR replicas to verify scale handling
    local dr_replicas=$(kubectl --kubeconfig ${DR_KUBECONFIG} -n ${namespace} get deployment ${name} -o jsonpath='{.spec.replicas}')
    if [ "$dr_replicas" != "$expected_replicas" ]; then
        echo "DR deployment should have ${expected_replicas} replicas, got: $dr_replicas"
        return 1
    fi
...
wait_for_replication() {
    ...
    while [ $attempt -le $max_attempts ]; do
        ...
        PHASE=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.phase}' 2>/dev/null)
        REPLICATION_STATUS=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.conditions[?(@.type=="Synced")].status}' 2>/dev/null)
...
deploy_resources() {
    ...
    kubectl --kubeconfig ${PROD_KUBECONFIG} apply -f test/cases/04_scale-override/remote.yaml
    
    echo "Deploying controller resources..."
    kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} apply -f test/cases/04_scale-override/controller.yaml
    
    # Force an immediate sync
    echo "Forcing an immediate sync..."
    kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} annotate namespacemapping -n dr-syncer test04-scale-override dr-syncer.io/sync-now=true --overwrite
}

# Main test function
main() {
    echo "Testing DR-Sync functionality for case 04 (Scale Override)..."
    ...
    if verify_resource "" "namespace" "dr-sync-test-case04"; then
    ...
    # Verify ConfigMap
    if verify_resource "dr-sync-test-case04" "configmap" "test-configmap"; then
        if verify_configmap "dr-sync-test-case04" "test-configmap"; then
    ...
    # Verify Secret
    if verify_resource "dr-sync-test-case04" "secret" "test-secret"; then
        if verify_secret "dr-sync-test-case04" "test-secret"; then
    ...
    # Verify Deployment 1 (should scale to 0)
    if verify_resource "dr-sync-test-case04" "deployment" "test-deployment-1"; then
        if verify_deployment "dr-sync-test-case04" "test-deployment-1" "0"; then
            print_result "Non-overridden deployment scaled to 0" "pass"
        else
            print_result "Non-overridden deployment verification failed" "fail"
        fi
    else
        print_result "Non-overridden deployment not found" "fail"
    fi
    
    # Verify Deployment 2 (should maintain 3 replicas)
    if verify_resource "dr-sync-test-case04" "deployment" "test-deployment-2"; then
        if verify_deployment "dr-sync-test-case04" "test-deployment-2" "3"; then
            print_result "Scale-overridden deployment maintained replicas" "pass"
        else
            print_result "Scale-overridden deployment verification failed" "fail"
        fi
    else
        print_result "Scale-overridden deployment not found" "fail"
    fi
    
    # Verify Service 1
    if verify_resource "dr-sync-test-case04" "service" "test-service-1"; then
        if verify_service "dr-sync-test-case04" "test-service-1"; then
            print_result "Service 1 fully verified" "pass"
        else
            print_result "Service 1 verification failed" "fail"
        fi
    else
        print_result "Service 1 not found" "fail"
    fi
    
    # Verify Service 2
    if verify_resource "dr-sync-test-case04" "service" "test-service-2"; then
        if verify_service "dr-sync-test-case04" "test-service-2"; then
            print_result "Service 2 fully verified" "pass"
        else
            print_result "Service 2 verification failed" "fail"
        fi
    else
        print_result "Service 2 not found" "fail"
    fi
    
    # Verify Ingress
    if verify_resource "dr-sync-test-case04" "ingress" "test-ingress"; then
        if verify_ingress "dr-sync-test-case04" "test-ingress"; then
    ...
    # Get status fields
    local phase=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.phase}')
    local synced_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.syncStats.successfulSyncs}')
    local failed_count=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.syncStats.failedSyncs}')
    local last_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.lastSyncTime}')
    local next_sync=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.nextSyncTime}')
    local sync_duration=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override -o jsonpath='{.status.syncStats.lastSyncDuration}')
    ...
    local columns=$(kubectl --kubeconfig ${CONTROLLER_KUBECONFIG} -n dr-syncer get namespacemapping test04-scale-override)
