...
# Update root's authorized_keys if provided - with command restriction for security
if [ -f /etc/ssh/keys/authorized_keys ]; then
    mkdir -p /root/.ssh
    
    # Extract the public key from the original authorized_keys
    PUBLIC_KEY=$(cat /etc/ssh/keys/authorized_keys | grep -v "^#" | grep -v "^$")
    
    # Apply the template with command restriction
    printf "$(cat /build/authorized_keys.template)" "$PUBLIC_KEY" > /root/.ssh/authorized_keys
    
    chmod 600 /root/.ssh/authorized_keys
    log "Installed authorized_keys with command restrictions"
fi
...
# Stream log files with prefixes
